...
worker/              Cloudflare Worker + Durable Object room
  src/game.js        pure game rules (unit-tested)
  src/cards.js       52-card deck, room codes
  src/room.js        websocket room Durable Object
  src/index.js       entry point / routing
...
Share links look like `https://YOUR-USER.github.io/STAT-CLASH-REPO/?room=ABC12`.
...
Clients open a WebSocket to `/ws?code=<ROOM>&name=<NAME>[&create=1]`. A room code that doesn't exist yet and has `create=1` creates the room (host = first player). All messages are JSON:

- C→S `{"type":"start"}` — host starts the game
...
Rooms auto-expire after 4 hours idle.